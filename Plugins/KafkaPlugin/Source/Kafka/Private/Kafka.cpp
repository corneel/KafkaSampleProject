...
void FKafkaModule::SetConsumerTopic(FName ftopic, int par, TMap<FString, FString>& ConfigurationSettings)
{
	...
	bool pushbackset = false;
	if (ConfigurationSettings.Contains("topic.auto.offset.reset"))
	{
...
bool FKafkaModule::Consume(FString& Err, FString& Str, int timeout)
{
	Str = Err = "";
	RdKafka::Message *message = kafc->consume(timeout);
...
bool FKafkaModule::Consume(FString& Err, TArray<FString>& StrArr, int timeout, int retries)
{
	class RdKafka::Metadata *metadata;
	lastErrorCode = kafc->metadata(topic != NULL, topic, &metadata, 500);
	if (lastErrorCode != RdKafka::ERR_NO_ERROR)
	{
		Err = ErrorMap.GetError(lastErrorCode);
		return false;
	...
	while (run && retry < retries)
	{
		if (Consume(Err, str, timeout))
		{
			StrArr.Add(str);
			anydata = true;
		}
